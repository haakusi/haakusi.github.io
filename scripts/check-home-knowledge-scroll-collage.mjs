...
test('section 05 owns one semantic counter-moving knowledge preview', async () => {
    ...
    assert.equal((home.match(/data-knowledge-rail="(?:forward|reverse)"/g) ?? []).length, 2);
    assert.equal((home.match(/data-knowledge-motion-item/g) ?? []).length, 12);
    ...
    assert.equal(new Set(sources).size, 12, 'motion preview should not repeat imagery');
...
test('knowledge motion has entry, convergence, and archive phases', () => {
    ...
    assert.ok(entry.railA < -300 && entry.railB > 300 && entry.scale < 0.9);
    assert.ok(Math.abs(focus.railA) < Math.abs(entry.railA) && Math.abs(focus.railB) < Math.abs(entry.railB));
...
test('desktop motion is bounded and all other modes keep the archive static', () => {
    assert.match(css, /body\.profile-motion-enhanced\s+\.profile-notes\s*\{[^}]*min-height:\s*clamp\([^}]*205svh/);
